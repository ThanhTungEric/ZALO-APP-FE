...
const ChatBox = ({ route }) => {
    ...
    const handleFileDownload = async (fileUrl) => {
        try {
            // Thực hiện việc tải xuống file từ fileUrl
            // Ví dụ sử dụng thư viện Expo FileSystem
            const { uri: fileUri } = await FileSystem.downloadAsync(
                fileUrl,
                FileSystem.documentDirectory + 'downloaded_file'
            );
            console.log('File đã được tải xuống tại:', fileUri);
            // Hiển thị thông báo hoặc thực hiện các xử lý khác sau khi tải xuống thành công
        } catch (error) {
            console.error('Lỗi khi tải xuống file:', error);
            // Hiển thị thông báo lỗi hoặc thực hiện các xử lý khác khi có lỗi xảy ra
        }
    };
